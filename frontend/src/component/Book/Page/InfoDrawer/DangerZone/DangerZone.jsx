...
const DangerZone = observer((props) => {
  ...
  const handlePatchActive = (value) => {
    ...
    patchEntry(value)
      .then(() => {
        ...
        notification.error({
          description: `Song #${props.id} has been ${
            isActive ? "a" : "una"
          }rchived.`,
          icon: <FolderOpenOutlined style={{ color: "#000" }} />,
        });
      })
      .catch((error) => {
        authStore.logout();
  ...
  const handleDeleteSong = () => {
    ...
    deleteEntry()
      .then(() => {
        notification.error({
          description: `Song #${props.id} has been deleted.`,
          icon: <DeleteOutlined style={{ color: "#6E0F1C" }} />,
        });
        props.setPageHasChanged("true");
      })
      .catch((error) => {
  ...
  return (
    <>
      ...
      >
        <Button block icon={<DeleteOutlined />} type="primary" danger>
          Delete this song
        </Button>
      </Popconfirm>
    </>
